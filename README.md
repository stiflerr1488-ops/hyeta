...
2. Соберите portable-версию:
   ...
   ```
   Команда автоматически удалит старые portable `.exe` (в корне проекта и в `dist/`), затем соберёт новый.
3. Готовый portable-файл вида `hyeta-visual-editor 0.1.0.exe` будет автоматически перемещён в корень проекта (основную папку).
4. Этот файл можно просто скопировать на другой Windows-ПК и запускать двойным кликом — установка не требуется.

#### Запуск сборки двойным кликом
...
- `run-without-build.bat` — быстрый запуск приложения без упаковки (для разработки и быстрых правок).
- После `build-portable-ready.bat` итоговый portable `.exe` лежит в корне проекта рядом с bat-файлами.

> Если portable-сборка падает: закройте запущенный portable `.exe`, затем повторите сборку. Скрипт удаляет/заменяет старый exe и это не получится, пока файл занят.
...
## Запуск БЕЗ создания билда (самый быстрый вариант)
- Через терминал:
  ```bash
  npm install
  npm start
  ```
- Или двойным кликом по `run-without-build.bat` в корне проекта.

Это запускает Electron напрямую из исходников, без `electron-builder`, поэтому старт обычно заметно быстрее, чем полная пересборка portable.
